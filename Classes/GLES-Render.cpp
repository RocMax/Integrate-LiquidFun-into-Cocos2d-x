...
void GLESDebugDraw::DrawParticles(const b2Vec2 *centers_old, float32 radius, const b2ParticleColor *colors, int32 count)
{
     ...
     */
    const float particle_size_multiplier = 4;  // no falloff
    ...
    {
        glVertexAttrib4f(GLProgram::VERTEX_ATTRIB_COLOR, 0, 1, 1, global_alpha);
        mShaderProgram->setUniformLocationWith4f(mColorLocation, 0, 1, 1, global_alpha);
    }
